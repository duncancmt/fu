...
contract FU is ERC20Base, TransientStorageLayout {
    ...
    function _applyWhaleLimit(Shares shares, Shares totalShares_) private pure returns (Shares, Shares) {
        Shares limit = (totalShares_ - shares).div(Settings.ANTI_WHALE_DIVISOR - 1) - ONE_SHARE;
        if (shares > limit) {
        ...
        returns (Shares, Shares, Shares)
    {
        ...
        Shares firstLimit = (totalShares_ - sharesHi).div(Settings.ANTI_WHALE_DIVISOR - 1) - ONE_SHARE;
        if (sharesHi > firstLimit) {
            Shares secondLimit = (totalShares_ - sharesHi - sharesLo).div(Settings.ANTI_WHALE_DIVISOR - 2) - ONE_SHARE;
    ...
    function whaleLimit(address potentialWhale) external view override returns (uint256) {
        return Tokens.unwrap(
            _$().totalSupply.mul(uint256(uint160(potentialWhale)) / Settings.ADDRESS_DIVISOR).div(
                Settings.CRAZY_BALANCE_BASIS * Settings.ANTI_WHALE_DIVISOR
            )
        );
    }
